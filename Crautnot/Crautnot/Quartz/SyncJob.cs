...
public class SyncJob : IJob {
    ...
    public async Task Execute(IJobExecutionContext context) {
        try {
            ...
            if (_sendWith48) {
                await scope.ServiceProvider.GetService<SynchronizeService>().Synchronize(currentPageSize, _categoryId);
                NextIteration(false);
            }
            else {
                await scope.ServiceProvider.GetService<SynchronizeService>().Synchronize(currentPageSize, null);
                NextIteration(true);
            }
